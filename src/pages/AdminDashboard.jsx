...
export default function AdminDashboard() {
  ...
  // NEW: Store the latest aggregated stats for KPIs
  ...
  // Scroll to stress section when test starts
  useEffect(() => {
  ...
  const loadAllData = async () => {
    try {
      setLoading(true);
      const [initData, analyticsRes] = await Promise.all([ fetchInitialData(), getAdminAnalytics() ]);
      setApis(initData.apis); setTiers(initData.tiers); setKeys(initData.keys);
  ...
  // ==========================================
  // STRESS TEST LOGIC (PARSING & STREAMING)
  // ==========================================
  
  const openStressModal = (api) => {
    setStressTargetApi(api);
    const validKey = keys.find(k => k.api_id === api.id && k.enabled)?.key_value || "";
    setStressForm({ num_users: 10, spawn_rate: 2, duration: 15, api_key: validKey });
    setIsStressConfigOpen(true);
  };
  ...
  const runStressTest = async (e) => {
    ...
    setIsStressConfigOpen(false);
    setIsTestRunning(true);
    
    // Reset Data
    setStressLogs(["Initializing Locust Swarm..."]);
    setStressChartData([]);
    setStressStats({ total_requests: 0, current_rps: 0, avg_latency: 0, max_latency: 0, fail_rate: 0 });
    ...
    try {
      const response = await fetch(`${BASE_URL}/stress/start`, {
        ...
        body: JSON.stringify({
          ...
          target_endpoint: stressTargetApi.endpoint,
          api_key: stressForm.api_key,
          num_users: parseInt(stressForm.num_users),
          spawn_rate: parseInt(stressForm.spawn_rate),
          duration: parseInt(stressForm.duration)
        })
      });
        ...
        lines.forEach(line => {
          try {
            ...
            if (data.log) {
              ...
              // Line Format: "Aggregated  10  0(0.00%) |  50  31  210  33 |  2.35  0.00"
              if (data.log.startsWith("Aggregated")) {
                const parts = data.log.split("|");
                if (parts.length === 3) {
                  // Part 0: "Aggregated  10  0(0.00%)"
                  const reqPart = parts[0].trim().split(/\s+/);
                  const totalReqs = parseInt(reqPart[1]);
                  const failStr = reqPart[2]; // "0(0.00%)"
                  const failRate = parseFloat(failStr.split("(")[1].replace("%)", ""));

                  // Part 1: "  50  31  210  33 " (Avg, Min, Max, Median)
                  const latencyPart = parts[1].trim().split(/\s+/); 
                  const avgLat = parseFloat(latencyPart[0]);
                  const maxLat = parseFloat(latencyPart[2]);

                  // Part 2: "  2.35  0.00" (Req/s, Fail/s)
                  const ratePart = parts[2].trim().split(/\s+/);    
                  const rps = parseFloat(ratePart[0]);

                  const newDataPoint = {
                    time: new Date().toLocaleTimeString(),
                    rps: rps,
                    failures: parseFloat(ratePart[1]),
                    avg_latency: avgLat
                  };
                  
                  if (!isNaN(rps)) {
                    setStressChartData(prev => [...prev, newDataPoint]);
                    // Update KPI Stats Live
                    setStressStats({
            ...
            if (data.status === "completed") {
              setStressLogs(prev => [...prev, ">>> TEST COMPLETED <<<"]);
      ...
      setStressLogs(prev => [...prev, `ERROR: ${error.message}`]);
      setIsTestRunning(false);
    }
  };

  // Helpers
  const getFilteredKeys = () => { /* ... existing logic ... */ return keys.filter(k => { if (keyStatusFilter === "ACTIVE" && !k.enabled) return false; if (keyStatusFilter === "REVOKED" && k.enabled) return false; if (keySearch) { const apiName = apis.find(a => a.id === k.api_id)?.name || ""; const searchLower = keySearch.toLowerCase(); return k.key_value.toLowerCase().includes(searchLower) || apiName.toLowerCase().includes(searchLower); } return true; }); };
  ...
  if (loading && !analytics.length) return <div style={s.centerScreen}>Loading Dashboard...</div>;
    ...
    <div style={s.layout}>
      ...
      <main style={s.main}>
        ...
        <div style={s.content}>
          ...
          {activeTab === 'overview' && (
            <div style={s.grid}>
              <div style={s.cardLarge}>
                <h3 style={s.cardHeader}>Global Traffic Volume</h3>
              ...
              <div style={s.cardLarge}>
                <h3 style={s.cardHeader}>Latency & Errors</h3>
                <div style={{ height: 300 }}>
          ...
          {/* 2. API MANAGEMENT & STRESS RESULTS */}
          {activeTab === 'apis' && (
            <>
              <div style={s.toolbar}><button style={s.primaryBtn} onClick={() => openApiModal()}>+ Register Service</button></div>
              <div style={s.tableCard}>
                <table style={s.table}>
                  <thead><tr style={s.thRow}><th style={s.th}>ID</th><th style={s.th}>Service Name</th><th style={s.th}>Endpoint</th><th style={s.th}>Status</th><th style={s.thRight}>Actions</th></tr></thead>
                  <tbody>{apis.map(api => (<tr key={api.id} style={s.tr}><td style={s.td}>{api.id}</td><td style={s.td}><strong>{api.name}</strong></td><td style={s.td}><code style={s.code}>{api.endpoint}</code></td><td style={s.td}><span style={api.enabled ? s.badgeGreen : s.badgeRed}>{api.enabled ? "Active" : "Disabled"}</span></td>
                        <td style={s.tdRight}>
                          <button style={s.actionBtnBlue} onClick={() => openStressModal(api)}><Icons.Zap size={12} style={{marginRight: 4}}/> Stress Test</button>
                          <button style={s.iconBtn} onClick={() => openApiModal(api)}><Icons.Edit2 size={16}/></button>
                          <button style={s.iconBtnDestructive} onClick={() => removeApi(api.id)}><Icons.Trash size={16}/></button>
                        </td></tr>))}</tbody>
                </table>
              </div>
              
              {/* === EMBEDDED STRESS RESULTS SECTION === */}
              {(stressChartData.length > 0 || isTestRunning) && (
                <div style={s.stressSection} ref={stressSectionRef}>
                  <div style={s.stressHeader}>
                    <div style={{display:'flex', alignItems:'center', gap: 10}}>
                      <div style={isTestRunning ? s.pulseDot : s.dotGreen}></div>
                      <h3 style={{fontSize: 18, color: 'white', margin: 0}}>
                        {isTestRunning ? "🔥 Stress Test In Progress..." : "✅ Stress Test Completed"}
                      </h3>
                    </div>
                    {isTestRunning && <span style={{color: '#94a3b8', fontSize: 13}}>Streaming Data...</span>}
                  </div>

                  {/* NEW: KPI STATS CARDS */}
                  <div style={s.kpiGrid}>
                    <KpiCard label="Total Requests" value={stressStats.total_requests} icon={<Icons.Activity size={18} color="#3b82f6"/>} />
                    <KpiCard label="RPS (Current)" value={stressStats.current_rps} icon={<Icons.Zap size={18} color="#eab308"/>} />
                    <KpiCard label="Avg Latency" value={`${stressStats.avg_latency} ms`} icon={<Icons.Clock size={18} color="#a855f7"/>} />
                    <KpiCard label="Max Latency" value={`${stressStats.max_latency} ms`} icon={<Icons.AlertCircle size={18} color="#f97316"/>} />
                    <KpiCard label="Error Rate" value={`${stressStats.fail_rate}%`} icon={<Icons.XCircle size={18} color="#ef4444"/>} isDanger={stressStats.fail_rate > 0} />
                  </div>

                  {/* LIVE CHARTS */}
                  <div style={s.stressGrid}>
                    <div style={s.stressCard}>
                      <h4 style={s.stressCardTitle}>Requests per Second (RPS)</h4>
                      <ResponsiveContainer width="100%" height={200}>
                        <AreaChart data={stressChartData}>
                          ...
                          <Area type="monotone" dataKey="failures" stroke="#ef4444" fill="#ef4444" strokeWidth={2} />
                        </AreaChart>
                      </ResponsiveContainer>
                    </div>

                    <div style={s.stressCard}>
                      <h4 style={s.stressCardTitle}>Response Time (ms)</h4>
                      <ResponsiveContainer width="100%" height={200}>
                        <AreaChart data={stressChartData}>
                          <defs>
                          ...
                          <YAxis stroke="#94a3b8" fontSize={10} />
                          <Tooltip contentStyle={{background: '#1e293b', border: 'none', color: '#fff'}} />
                          <Area type="monotone" dataKey="avg_latency" stroke="#3b82f6" fillOpacity={1} fill="url(#colorLat)" strokeWidth={2} />
                        </AreaChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
                  ...
                  <div style={s.terminalBox}>
                    <div style={s.terminalTitle}>Execution Logs</div>
